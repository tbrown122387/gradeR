...
## Assigning Point Values to Tests

Each `test_that()` block becomes one graded question on Gradescope, and by default every `expect_*()` call inside it is worth 1 point. If you want some questions to be worth more than others, you set that **inside the individual `expect_*()` calls**, not in the name of the `test_that()` block.

Do this by passing a `label` argument to the `expect_*()` call, with a tag like `[2pts]` or `(2pts)` (the trailing `s` is optional) somewhere in the string:
...
This block is worth 4 points total (1 + 3), and the `label` text (with the `[Npts]` tag stripped off) becomes the message shown to the student for that criterion on Gradescope. If an `expect_*()` call has no `label`, it defaults to a message like "Criterion 1" and is worth 1 point.

A common mistake is to put the `[Npts]` tag in the *name* of the `test_that()` block instead, e.g. `test_that("p1a (visible) [2pts]", ...)`. That has no effect on scoring — the `test_that()` name only controls visibility (`(visible)`, `(hidden)`, etc.), never points.

Before you upload a test file to Gradescope, run `gradeR::getTestScriptReport("your_test_file.R")` on it locally. It prints a breakdown of every test, its criteria, and their point values, along with a running total — use that total as the "max score" you enter for the assignment on Gradescope.
...
7. Make sure the max score you enter on Gradescope matches the **total point value** of your test file, not just the number of `test_that()` blocks. If every `expect_*()` call is left at its default of 1 point, these are the same number — but as soon as you customize any point values (see "Assigning Point Values to Tests" above), they diverge. Run `gradeR::getTestScriptReport("your_test_file.R")` locally to get the correct total before entering it on Gradescope.
