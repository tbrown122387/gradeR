...
All you have to do is ask your students to assign variables to specific names. Otherwise, each student might use different names for his/her variables, and that would make testing all of the assignments very challening. 
...
That's it. However, there are also some recommended warnings you provide as well. The students will have to follow a few rules so that their assignments are gradeable. Their code should not:
...
it is unlikely that path will exist on your machine, and so `calcGrades` will tell you that the assignment is not gradeable. Instead, the student should write code like this
...
In this way, their submissions will not require any intervention on the instructor's part. 
...
This package relies on that [`testthat` package](https://testthat.r-lib.org/). `gradeR` makes use of `testthat`'s  ["expectations"](https://testthat.r-lib.org/reference/index.html). Most of the time you will be testing if certain variables are what you expect them to be. 
